...
<div class="px-2 my-4">
    ...
    <a href="{{ url('personal/payrolls') . '/' . $payroll['id'] }}">
        ...
        <div class="mt-2 bg-white shadow px-4 py-3 border-l-4 {{ $borderColor }}">
            ...
            <p class="text-xl font-semibold">Period {{ $payroll['period_id'] }} <span class="font-normal text-xs"></span></p>
            @else
            <p class="text-xl font-semibold">Special Payroll<span class="font-normal text-xs">(May 11 - May 25, 2023)</span></p>
            @endif
            ...
            <div class="flex items-center mb-2 text-blue-400">
                ...
                <h2 class="ml-1 text-sm text-gray-800">{{ $payroll['type'] == "regular" ? "Salary Disbursement" : $payroll['type'] }} - {{ $status }}</h2>
            </div>
        </div>
        @endforeach
    </a>
